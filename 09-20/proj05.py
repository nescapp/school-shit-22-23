...
# vérifie que l'utilisateur rentre bien un nombre entre min et max
...
def Loterie(gain: int):  # fonction principale
    if input("Appuyez sur entrée pour jouer ou écrivez 'exit' pour quitter : ").lower() != "exit":
        nombre_chiffres = Verify_input(
            "Entrez le nombre de chiffres à cocher : ", 2, 10)
        val_max = Verify_input(
            "Entrez la valeur maximum des chiffres à cocher : ", 5, 50)
        ...
        if Gagnant(ticket_joueur, ticket_gagnant):  # si le joueur gagne
            print("Vous avez remporté le prix de",
                  "{:,}".format(gain), "€ !!! Félicitation !!")
        else:  # si le joueur perd on double le gain
            print("Vous n'avez pas remporté le grand prix de", "{:,}".format(
                gain), f"€. Vous aurez plus de chance la prochaine fois. Vous aviez {Calcul_probabilite()}% de chance de remporter le gros lot.")
            Loterie(gain*2)
